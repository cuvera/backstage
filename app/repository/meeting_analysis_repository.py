...
class MeetingAnalysisRepository(BaseRepository):
    """Repository for meeting analysis data persistence."""
    ...
    @classmethod
    async def from_default(cls, collection_name: str = COLLECTION) -> "MeetingAnalysisRepository":
    ...
    async def save_analysis(self, analysis: MeetingAnalysis) -> Dict[str, Any]:
        ...
        logger.info(
            "[MeetingAnalysisRepository] Upserting analysis for tenant=%s session=%s",
            analysis.tenant_id,
            analysis.session_id,
        )
    ...
    async def get_analyses_by_session_ids(
        ...
        *, 
        ...
        """
        if not session_ids:
            logger.warning("[MeetingAnalysisRepository] No session IDs provided for batch fetch")
            return []
        ...
        try:
            ...
            logger.info(
                "[MeetingAnalysisRepository] Fetched %d analyses for %d session IDs",
                len(docs),
                len(session_ids)
            )
            return docs
            ...
            logger.error(
                "[MeetingAnalysisRepository] Error fetching analyses for session_ids %s: %s",
                session_ids,
                exc
            )
            return []