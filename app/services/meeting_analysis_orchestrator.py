...
from app.messaging.producers.transcription_producer import (
    publish_v1,
    publish_v2,
...
class MeetingAnalysisOrchestrator:
    ...
    async def analyze_meeting(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            mode = "online" if speaker_timeframes else "offline"
            ...
            # Validate transcription has segments before proceeding
            ...
            # Step 1.5: Publish V1 to RabbitMQ
            await publish_v1(
                meeting_id=meeting_id,
                tenant_id=tenant_id,
                platform=platform,
            ...
            logger.info(f"Step 1.5: Published V1 transcription to RabbitMQ - meeting_id={meeting_id}")
            ...
            ########################################################
            # Step 2: Transcription with TranscriptionService v2 #
            ########################################################
            logger.info(f"[Orchestrator] Step 2: Processing transcription V2 for meeting {meeting_id}")
            ...
            if transcription_v2:
                ...
                logger.info(f"[Orchestrator] Published V2 transcription to RabbitMQ - meeting_id={meeting_id}")
            ...
            try:
                # Determine mode from payload (might not be set if error occurred early)
                speaker_timeframes = payload.get("speakerTimeframes", [])
                mode = "online" if speaker_timeframes else "offline"
                ...
                logger.info(f"Published failure status to RabbitMQ - meeting_id={meeting_id}")
